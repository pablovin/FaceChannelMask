...
print ("shape:" + str(numpy.array(arousals).shape))
results = numpy.array(modelDimensional.classify(images))

arousalsOutput = results[0,:,0][0:3519]
valencesOutput = results[1,:,0][0:3519]

arousalsOutput = numpy.reshape(arousalsOutput,(3, -1))
valencesOutput = numpy.reshape(valencesOutput,(3, -1))

arousals = numpy.reshape(arousals[0:3519],(3, -1))
valences = numpy.reshape(valences[0:3519],(3, -1))



for a in range(3):
    arousalCCC = ccc_numpy(arousals[a,:], arousalsOutput[a,:])
    valenceCCC = ccc_numpy(valences[a,:], valencesOutput[a,:])
    print ("Bin: " + str(a) + "Arousal: " + str(arousalCCC)  + " - Valence: " + str(valenceCCC))


# hist = numpy.histogram(arousals, bins=3)[0]
#
# counts = numpy.apply_along_axis(hist, axis=0, arr=arousals)

# print ("arousals:" + str(arousals.shape))

# histA, edgeA = numpy.histogram(arousals, 3)
# histV, edgeV = numpy.histogram(valences, 3)
#
# print ("histogramA:" + str(histA))
# print ("histogramV:" + str(histV))
